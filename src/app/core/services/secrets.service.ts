...
interface State {
  ...
  data: Secret[];
  loading: Record<'create' | 'list', boolean> &
    Record<'delete' | 'update', ID[]>;
  now: number;
}
...
export class SecretsService {
  ...
  actor$ = toObservable(this.#authService.identity).pipe(
    switchMap((identity) =>
      createActor<BackendActor>({
        canisterId: environment.backendCanisterId,
        identity,
    ...
    return this.actor$.pipe(
      ...
      finalize(() =>
        this.#state.update((state) => ({
          ...
          loading: {
            ...state.loading,
            delete: state.loading.delete.filter((_id) => _id !== id),
          },
        })),
      ),
    );
  ...
  update(payload: SecretUpdate) {
    this.#state.update((state) => ({
      ...
      loading: {
        ...state.loading,
        update: [...state.loading.update, payload.id],
      },
    }));
    return this.actor$.pipe(
      ...
      switchMap((result) =>
        match(result)
          .with({ err: { notFound: P.nullish } }, () =>
            throwError(
              () => new Error(`The secret with ID ${payload.id} not found.`),
            ),
          )
          .otherwise(({ ok }) => of(ok)),
      ),
      finalize(() =>
        this.#state.update((state) => ({
          ...
          loading: {
            ...state.loading,
            update: state.loading.update.filter((id) => id !== payload.id),
          },
        })),
      ),
    );
  ...
  #initListObserving() {
    ...
    const stateReducer = (
      state: State,
      value: ExtractObservable<typeof list$>,
    ) =>
      match(value)
